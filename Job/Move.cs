...
internal class Move(PlayerState state) : JobBase(state)
{
    ...
    protected override void _PhysicsUpdate(double delta)
    {
        ...
        velocity.X = Mathf.MoveToward(velocity.X, direction * state.Properties.RunSpeed.Current, 
            state.Properties.FloorAcceleration.Current);
        velocity.Y += (float)delta * state.Properties.Gravity.Current;
