...
public partial class DamageNumber : Node2D
{
    ...
    [Export] private Color color = Colors.Red; // 设置伤害数字的颜色

    [Export] private float lifetime = 0.5f; // 设置伤害数字的显示时间

    [Export] private float moveDistance = 20; // 设置伤害数字的移动距离
    ...
    public override void _Ready()
    {
        ...
        _tween.TweenProperty(this, "scale", new Vector2(1.0f, 1.0f), 0.2f).SetTrans(Tween.TransitionType.Bounce)
            .SetDelay(0.2f);

        // 颜色变化效果
        _tween.TweenProperty(_label, "modulate", new Color(1.0f, 0.0f, 0.0f), 0.2f); // 从红色开始
        _tween.TweenProperty(_label, "modulate", new Color(1.0f, 1.0f, 1.0f), 0.3f).SetDelay(0.2f); // 变为白色
