...
{
    // 存储场景状态的字典
    private readonly Dictionary<string, SceneState> _sceneStates = new();
    ...
    public override void _Ready()
    {
        ...
        if (_currentScene == null) GD.PrintErr("Current scene is null!");

        // 加载过渡动画场景
        try
        {
            var transitionScene =
                GD.Load<PackedScene>("res://addons/Miros/Core/SceneTransitionStyle/FadeTransition.tscn");
    ...
    public async Task ChangeScene(string scenePath, Node2D transferNode = null, Vector2? entryPoint = null,
        bool playTransition = true)
    {
        try
        {
            ...
            // 播放过渡动画
            if (playTransition && _transition != null) await _transition.TransitionOut();
            ...
            // 切换场景
            var tree = GetTree();
            if (_currentScene != null) tree.Root.RemoveChild(_currentScene);
            ...
            if (_sceneStates.ContainsKey(scenePath))
            {
                ...
                foreach (var kvp in sceneState.NodeVisibility)
                {
                    ...
                    if (node != null) (node as Node2D).Visible = kvp.Value;
                }
            ...
            if (nodeToTransfer != null)
            {
                ...
                if (entryPoint.HasValue) nodeToTransfer.GlobalPosition = entryPoint.Value;
                GD.Print($"Transferred node {nodeToTransfer.Name} to new scene");
            }

            // 播放淡入动画
            if (playTransition && _transition != null) await _transition.TransitionIn();
    ...
    public void RecordNodeDestroyed(Node node)
    {
        ...
        if (!_sceneStates.ContainsKey(scenePath)) _sceneStates[scenePath] = new SceneState();
    ...
    {
        if (_sceneStates.ContainsKey(scenePath)) _sceneStates.Remove(scenePath);
    }
    ...
    {
        public HashSet<string> DestroyedNodes { get; } = new();
        public Dictionary<string, Vector2> NodePositions { get; } = new();
        public Dictionary<string, bool> NodeVisibility { get; } = new();
        public Dictionary<string, Dictionary<string, object>> CustomProperties { get; } = new();
        public Dictionary<string, bool> CollisionStates { get; } = new();
    }
}