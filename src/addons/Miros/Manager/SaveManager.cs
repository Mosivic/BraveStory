...
public partial class SaveManager : Node
{
	...
	private EncryptionConfig LoadEncryptionConfig()
	{
		...
		{
			// 从外部配置文件加载加密配置
			string configPath = "res://config/encryption_config.json";
	...
	public void SaveGame(string saveName)
	{
		try
		{
			...
			// 根据开关决定是否加密
			string finalData = _useEncryption ? EncryptData(json) : json;
			
			string savePath = SAVE_DIR + saveName + SAVE_EXTENSION;
	...
	private void ValidateSaveData(GameSaveData saveData)
	{
		...
		// 验证存档次数
		if (saveData.SaveCount < 1)
		{
			throw new Exception("Invalid save file: Invalid save count");
		}
	...
	public GameSaveData GetSaveInfo(string saveName)
	{
		try
		{
			...
			{
				// 如果解析失败，尝试解密
				string json = DecryptData(fileContent);
