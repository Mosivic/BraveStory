...
public partial class GameplayTagTreeView : Tree
{
    ...
    public void RefreshTree()
    {
        ...
        foreach (var tag in rootTags) CreateTagTreeItem(tag, _root);
    }
    ...
    private void CreateTagTreeItem(Tag tag, TreeItem parent)
    {
        ...
        // 递归添加子标签
        foreach (var childTag in _tagManager.GetDirectChildTags(tag)) CreateTagTreeItem(childTag, item);
    }
    ...
    private void ShowRenameDialog(TreeItem item)
    {
        ...
        dialog.TextChanged += newText => ValidateTagName(dialog, newText);
    ...
    private void OnDeleteConfirmed()
    {
        ...
        // 获取要删除的标签路径
        var tagPath = _itemToDelete.GetText(1);
    ...
    private void OnRenameSubmitted(string newName)
    {
        ...
        // 获取当前完整路径
        var oldPath = _itemToRename.GetText(1);
        ...
        {
            var parentPath = parent.GetText(1);
    ...
    {
        // 验证标签名称的有效性
        var isValid = !string.IsNullOrWhiteSpace(newText) &&
                      Regex.IsMatch(newText, @"^[a-zA-Z][a-zA-Z0-9_]*$");
    ...
    private void UpdateChildrenPaths(TreeItem item, string oldBasePath, string newBasePath)
    {
        ...
        while (child != null)
        {
            var oldChildPath = child.GetText(1);
            var newChildPath = oldChildPath.Replace(oldBasePath, newBasePath);
