...
public class StateTransitionContainer
{
    ...
    public void AddTransition(Tag fromJobSign, Tag toJobSign, Func<bool> condition = null,
        StateTransitionMode mode = StateTransitionMode.Normal)
    {
        var transition = new StateTransition(toJobSign, condition, mode);
        if (!_transitions.ContainsKey(fromJobSign)) _transitions[fromJobSign] = [];
        _transitions[fromJobSign].Add(transition);
    }

    public void AddAnyTransition(Tag toJobSign, Func<bool> condition = null,
        StateTransitionMode mode = StateTransitionMode.Normal)
    {
        var transition = new StateTransition(toJobSign, condition, mode);
    ...
    {
        if (!_transitions.TryGetValue(fromJobSign, out var rules)) return false;
    ...
    {
        if (!_transitions.TryGetValue(fromJobSign, out var rules)) return Enumerable.Empty<StateTransition>();
