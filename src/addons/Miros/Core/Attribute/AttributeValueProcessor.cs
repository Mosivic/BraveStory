...
public class AttributeValueProcessor
{
    ...
    /// <param name="baseAdditiveBonus">
    ///     基础附加值：默认0，应使用加法来修改。
    ///     一般用于增加固定数值的装备加成: 比如增加10点攻击力。
    ///     参考英雄联盟中的额外攻击力，若不需要区分基础值和基础附加值，无视此参数。
    /// </param>
    /// <param name="additiveBonus">
    ///     累加加成：默认0，应使用加法来修改，25%加成对应的修改参数应该为0.25。
    ///     将所有的累加加成求和之后作为一个加成值(1 + a1 + a2 + ... + an), 它们的收益是会"稀释"的。
    ///     一般用于增加百分比的装备加成: 比如增加25%伤害。
    /// </param>
    /// <param name="multiplicativeBonus">
    ///     累乘加成：默认1(需要你自己设置)，应使用乘法来修改，25%加成对应的修改参数应该为1.25。
    ...
    private void OnAttributeChanged(AttributeBase attribute, float attrOldValue, float attrNewValue)
    {
        ...
        if (_onPreValueChange != null) newValue = _onPreValueChange(this, oldValue, newValue);

        if (Math.Abs(oldValue - newValue) < float.Epsilon) return;
        ...
        AttributeBase maxValuePenalty = null)
    {
        ...
        if (baseAdditiveBonus != null) totalValue += baseAdditiveBonus.CurrentValue;

        if (totalValue != 0)
        {
            if (additiveBonus != null) totalValue *= 1 + additiveBonus.CurrentValue;

            if (multiplicativeBonus != null) totalValue *= multiplicativeBonus.CurrentValue;

            if (maxValuePenalty != null) totalValue *= 1 - maxValuePenalty.CurrentValue;
        }
