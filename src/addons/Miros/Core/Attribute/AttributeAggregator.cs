...
public class AttributeAggregator
{
    ...
    private List<Tuple<EffectState, Modifier>> _modifierCache =
        new List<Tuple<EffectState, Modifier>>();

    public AttributeAggregator(AttributeBase attribute, object owner)
    {
        ...
        foreach (var geSpec in gameplayEffects)
        {
            if (geSpec.IsActive)
            {
                foreach (var modifier in geSpec.Modifiers)
                {
                    ...
                    {
                        _modifierCache.Add(new Tuple<EffectState, Modifier>(geSpec, modifier));
    ...
    private void TryUnregisterAttributeChangedListen(EffectState ge, Modifier modifier)
    {
    ...
    private void TryRegisterAttributeChangedListen(EffectState ge, Modifier modifier)
    {
        if (modifier.MMC is AttributeBasedModCalculation mmc &&
            mmc.captureType == AttributeBasedModCalculation.GEAttributeCaptureType.Track)
        {
