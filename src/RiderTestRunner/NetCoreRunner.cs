...
public partial class
    NetCoreRunner : Node // for GodotXUnit use: public partial class Runner : GodotTestRunner. https://github.com/fledware/GodotXUnit/issues/8#issuecomment-929849478
{
    ...
    public override void _Ready()
    {
        ...
        foreach (var arg in OS.GetCmdlineArgs()) textNode.Text += Environment.NewLine + arg;
    ...
    private Assembly CurrentDomainOnAssemblyResolve(AssemblyLoadContext loadContext, AssemblyName assemblyName)
    {
        ...
        if (alreadyLoadedMatch != null) return alreadyLoadedMatch;
    ...
    {
        while (thread.IsAlive) await ToSignal(GetTree().CreateTimer(100), "timeout");
    }
}